...
impl FakeSupergfxd {
    async fn supported(&self) -> zbus::fdo::Result<Vec<u32>> {
        Ok(self.state.lock().unwrap().supported.clone())
    }

    async fn set_mode(&self, mode: u32) -> zbus::fdo::Result<u32> {
        let state = self.state.lock().unwrap();
        state.calls.lock().unwrap().push(mode);
...
fn calls(state: &Arc<Mutex<FakeState>>) -> Vec<u32> {
