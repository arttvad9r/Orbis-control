...
3. **#107 Battery owner/interface liveness** — source-complete: dynamic bounded non-mutating owner probe plus, with an owner confirmed, a fresh uncached `ChargeControlEndThreshold` read; peer-reported structural absence (UnknownObject/Interface/Property) is proven drift → `TemporarilyUnavailable`, other failures stay `Unknown`. Executed: unit contracts + `battery-mutation-vm` status generations (supported → owner-loss demote → restore) on this revision. Remaining: drift exercised through an actual mis-serving daemon (VM/live).
