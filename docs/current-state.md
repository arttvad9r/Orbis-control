...
| Fan reads | IMPLEMENTED / STORED ENABLED CARRIED — #109 open | Per-fan Session1 read exists; stored `FanCurveData.enabled` is carried through Session1/client/UI evidence (#116 source-complete) and rendered from `UiState` in the FansWindow; aggregate capability still CPU-centric (#109). |
