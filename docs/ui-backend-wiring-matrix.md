...
| Fan factory defaults | `AsusdFanCurveMutationBackend::reset_curves_to_defaults` | UI direct Hardware1 provider exists, then refreshes selected curve; currently blocked by fan error state | `ResetFanCurvesToDefaults`, same fan polkit action, `Applied` | Profile-wide reset |
...
| Panel Overdrive | `AsusArmouryPanelOverdriveProvider`; hardwared asusd setter + kernel current read | Provider/probe/Hardware1 exist; not in current `ProductionRuntime`/UiState | `SetPanelOverdrive`, polkit `...set-panel-overdrive`, `Applied` | Confirmed present |
...
| Factory Defaults | 1 | Staged profile-wide default reset intent | Button only stages local pending; actual reset is sent from Apply; current fan error state can disable it | Hardware1 `ResetFanCurvesToDefaults`, fan polkit, asusd reset + `FanCurveData` observation, `Applied` | `WIRED_PARTIAL` |
| Apply Custom Fan Curve / Apply Factory Defaults | 1 | Explicit fan mutation commit | Custom → worker; reset → direct Hardware1 provider; both source-complete, but normal production editor is blocked by failed profile read | See mutation matrix | `WIRED_PARTIAL` |
...
| Fan factory defaults | Code wired; UI blocked by read defect | GUI callback → Tokio task → `Hardware1FanDefaultsProvider` → direct Hardware1 | `ResetFanCurvesToDefaults` | `...set-fan-curve` | asusd profile-wide reset | fresh `FanCurveData(profile)`, require recognized curves; UI then requests selected curve refresh | `Applied` | None; staging button itself performs no write |
