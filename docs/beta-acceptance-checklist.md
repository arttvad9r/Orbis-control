...
- [x] #106: hosted GitHub Actions is explicitly optional/manual by single-owner project policy; local full flake checks are canonical executable evidence.
...
- [x] #114: required hosted checks are intentionally not used by single-owner project policy; local checks and explicit review are required instead.
...
- [x] Raw GPU/Fan/Panel/Aura writes are product/policy blocked; FA707NV keyboard backlight is the only newly promoted extended write and has live read-back evidence.
...
2. the exact candidate has executable green local Rust/Nix checks;
